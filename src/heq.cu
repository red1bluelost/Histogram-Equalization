...
__global__ void calcHist(unsigned char *input, unsigned int *histogram,
  ...
  if (idx_in_block < 256) shared_hist[idx_in_block] = 0;
  ...
  if (idx_in_block < 256) {
    ...
    if (value != 0) atomicAdd(&histogram[idx_in_block], value);
  }
}
...
__global__ void mapFromHist(unsigned char *input, unsigned int *histogram) {
  const int location =
      (blockIdx.y * TILE_SIZE + threadIdx.y) * TILE_SIZE * gridDim.x +
...
void gpu_function(unsigned char *data, unsigned int height,
                  unsigned int width) {
  ...
  // Zero the histogram
  checkCuda(cudaMemset((unsigned int *)(input_gpu + size), 0,
                       histogram_size * sizeof(unsigned int)));

  // Copy data to GPU
  checkCuda(cudaMemcpy(input_gpu, data, size * sizeof(unsigned char),
...
// Kernel Call
  ...
  calcHist<<<dimGrid, dimBlock>>>(input_gpu, (unsigned int *)(input_gpu + size),
                                  height, width);
  ...
  checkCuda(cudaMemcpy(data, input_gpu, size * sizeof(unsigned char),
...
void gpu_warmup(unsigned char *data, unsigned int height, unsigned int width) {
  ...
  // Copy data to GPU
  checkCuda(
      cudaMemcpy(input_gpu, data, size * sizeof(char), cudaMemcpyHostToDevice));
  ...
  warmup<<<dimGrid, dimBlock>>>(input_gpu, output_gpu);
  // calculate the histogram bins
  calcHist<<<dimGrid, dimBlock>>>(input_gpu, (unsigned int *)(input_gpu + size),
                                  height, width);
  ...
  // Retrieve results from the GPU
  checkCuda(cudaMemcpy(data, output_gpu, size * sizeof(unsigned char),
                       cudaMemcpyDeviceToHost));
